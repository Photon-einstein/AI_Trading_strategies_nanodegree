...
def compute_BM_Perf(total_returns):
    ...
    stand_dev_daily_return   = daily_mean[['SP&500']].describe().iloc[2,:] * pow(252,1/2)
...
def compute_strat_perf(total_returns, cum_returns, calendar_returns, trading_strategy, model_name):    
    ...
    stand_dev_daily_return   = daily_mean[[f'{model_name}_Return']].describe().iloc[2,:] * pow(252,1/2)
...
def plot_optimal_cluster_point(sse, k_range):
    ...
    plt.scatter(k_normalized[elbow_index], sse_normalized[elbow_index], color='red', s=100, zorder=5, label='Elbow Point')
