...
def create_hist_prices(start_date="2000-01-01", end_date="2024-05-01"):
    ...
    historical_prices = yf.download(
        sp500_tickers, start=start_date, end=end_date, auto_adjust=False, threads=False
    )

    # Filter and keep only columns where the first level of the MultiIndex is 'Adj Close'
    historical_prices = historical_prices.loc[
        :, historical_prices.columns.get_level_values(0) == "Adj Close"
    ]
    ...
    valid_tickers_mask = ticker_counts[
        ticker_counts >= MIN_REQUIRED_NUM_OBS_PER_TICKER
    ].index
...
def compute_BM_Perf(total_returns):
    ...
    daily_mean = pd.DataFrame(
        total_returns.loc[:, "F_1_d_returns"].groupby(level="Date").mean()
    )
    ...
    stand_dev_daily_return = daily_mean[["SP&500"]].describe().iloc[2, :] * pow(
        252, 1 / 2
    )
    ...
    calendar_returns = pd.DataFrame(
        ann_returns["SP&500"].groupby(daily_mean.index.get_level_values(0).year).last()
    )
...
def compute_strat_perf(
    ...
    total_returns[f"{model_name}_Return"] = (
        total_returns["F_1_d_returns"] * total_returns["Position"]
    )

    # Compute the daily mean of all stocks. This will be our equal weighted benchmark
    daily_mean = pd.DataFrame(
        total_returns.loc[:, f"{model_name}_Return"].groupby(level="Date").mean()
    )

    # Convert daily returns to cumulative returns
    cum_returns.loc[:, f"{model_name}_Return"] = pd.DataFrame(
        (daily_mean[[f"{model_name}_Return"]] + 1).cumprod()
    )
    ...
    average_daily_return = (
        daily_mean[[f"{model_name}_Return"]].describe().iloc[1, :] * 252
    )
    stand_dev_daily_return = daily_mean[[f"{model_name}_Return"]].describe().iloc[
        2, :
    ] * pow(252, 1 / 2)
    ...
    calendar_returns.loc[:, f"{model_name}_Return"] = pd.DataFrame(
        ann_returns[f"{model_name}_Return"]
        .groupby(daily_mean.index.get_level_values(0).year)
        .last()
    )
...
def plot_optimal_cluster_point(sse, k_range):
    ...
    plt.plot(
        [0, 1], [sse_normalized[0], sse_normalized[-1]], "r--", label="Reference Line"
    )
    ...
    for i in range(len(k_normalized)):
        ...
        plt.plot(
            [k_normalized[i], projected_point[0]],
            [sse_normalized[i], projected_point[1]],
            "k:",
            alpha=0.7,
        )
    ...
    plt.scatter(
        k_normalized[elbow_index],
        sse_normalized[elbow_index],
        color="red",
        label="Elbow Point",
    )
