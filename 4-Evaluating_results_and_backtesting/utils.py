...
def plot_returns(returns):
    ...
    ax.fill_between(
        drawdowns.index, drawdowns, label="Drawdowns", color="red", alpha=0.3
    )
...
def plot_rolling_mean(cumulative_returns, rolling_mean):
    ...
    ax.plot(cumulative_returns - 1, label="Cumulative Returns")
    ax.plot(
        rolling_mean, label="Rolling Mean", linestyle="--", linewidth=1, color="green"
    )
